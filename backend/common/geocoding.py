...
def buscar_lat_lng_endereco(cep=None, endereco=None):
    ...
    # 1. Busca direta por CEP no BrasilAPI v2 (rápido, com coordenadas geográficas diretas)
    endereco_busca = ""
    if cep_clean and len(cep_clean) == 8:
        try:
            resp_bapi = requests.get(f"https://brasilapi.com.br/api/cep/v2/{cep_clean}", timeout=(2, 3))
            if resp_bapi.status_code == 200:
                bdata = resp_bapi.json()
                coords = bdata.get('location', {}).get('coordinates', {})
                if coords.get('latitude') and coords.get('longitude'):
                    return round(float(coords['latitude']), 6), round(float(coords['longitude']), 6)
                if bdata.get('street') and bdata.get('city'):
                    endereco_busca = f"{bdata.get('street')}, {bdata.get('neighborhood', '')}, {bdata.get('city')} - {bdata.get('state', '')}, Brasil"
        except Exception:
            pass

    # 2. Busca direta por CEP no Nominatim
    if cep_clean and len(cep_clean) == 8:
        try:
