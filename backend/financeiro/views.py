...
def api_salvar_linha(request):
    """Atualiza ajustes manuais de uma linha de manifesto (valor_extra, localidade_extra, observacao)."""
    if not verificar_permissao_financeiro(request.user):
        return JsonResponse({'sucesso': False, 'erro': 'Sem permissão.'}, status=403)
    ...
    try:
        ...
        if 'valor_extra' in data:
            linha.valor_extra = Decimal(str(data['valor_extra'] or 0))
        if 'localidade_extra' in data:
            linha.localidade_extra = (data['localidade_extra'] or '').strip().upper()
        if 'observacao' in data:
            linha.observacao = data['observacao']
...
def api_salvar_resumo(request):
    ...
    try:
        ...
        if 'valor_pedagio' in data:
            resumo.valor_pedagio = Decimal(str(data['valor_pedagio'] or 0))
        if 'valor_desconto' in data:
            resumo.valor_desconto = Decimal(str(data['valor_desconto'] or 0))
        if 'observacao' in data:
            resumo.observacao = data['observacao']
...
def api_salvar_tarifa(request):
    ...
    try:
        ...
        v_diaria = Decimal(str(data.get('valor_diaria', 0)))
        v_entrega = Decimal(str(data.get('valor_por_entrega', 5)))
        v_coleta = Decimal(str(data.get('valor_por_coleta', 10)))

        tarifa, created = TarifaAgregado.objects.update_or_create(
            filial=filial,
            defaults={
        ...
        return JsonResponse({
            'sucesso': True,
            'mensagem': f'Tarifa da filial {filial.nome} salva com sucesso!'
        })
    except Exception as e:
        return JsonResponse({'sucesso': False, 'erro': str(e)}, status=500)
